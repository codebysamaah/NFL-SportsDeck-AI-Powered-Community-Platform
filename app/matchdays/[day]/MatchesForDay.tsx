...
function MatchCard({ match }: { match: Match }){
    ...
    return (
        <div className="group relative flex items-center gap-4 px-5 py-4 rounded-xl bg-[var(--bg-card)] border border-[var(--border)] hover:border-slate-500/70 hover:bg-[var(--bg-secondary)] transition-all duration-200">
        ...
        <div className="w-28 shrink-0 text-right hidden sm:block">
            <p className="text-[11px] font-medium text-[var(--text-primary)] truncate">{match.stage}</p>
            <p className="text-[11px] text-[var(--text-primary)] truncate mt-0.5">{venue ?? "TBD"}</p>
            <p className="text-[11px] text-[var(--text-primary)] truncate mt-0.5">Matchday {match.matchday}</p>
        </div>
    </div>
  );
}
...
export default function MatchesPage(){
    ...
    const fetchMatches = async (page: number) => {
        setLoading(true);
        try {
            const res = await fetch(`/api/matches/matchday/${day}?page=${page}`);
    ...
    return (
        <div className="relative min-h-screen text-[var(--text-primary)] flex flex-col">
            ...
            <div className="fixed inset-0 bg-black/73" />

            {/* ── header ── */}
            <div className="border-b border-slate-800 bg-[var(--bg-primary)] backdrop-blur sticky top-0 z-10">
                <div className="max-w-3xl mx-auto px-4 py-4 flex items-center justify-between">
                <div>
                    <h1 className="text-4xl font-black tracking-tight text-[var(--text-primary)]">Matches for Day {day}</h1>
                    {!error && (
            ...
            <div className="flex-1 max-w-3xl w-full mx-auto px-4 py-6 space-y-7 z-10">
                {error && (
                    <div className="flex items-center gap-3 p-4 rounded-xl bg-red-900/50 border border-red-700/40 text-slate-200 text-sm">
