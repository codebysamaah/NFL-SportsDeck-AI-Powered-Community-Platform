...
import { STAGE_ENUM } from "@/utils/stages"

const CACHE_TTL_MS = 60 * 60 * 1000; 

// Get all matches for a particular stage in league
export async function GET(request, context){
    let { stage } = await context.params;

    if (!stage) {
        return NextResponse.json(
            { error: "Please enter a stage." },
            { status: 400 }
        );
    }
    ...
    if (!STAGE_ENUM.includes(stage)) {
        return NextResponse.json(
            { error: "Please enter a valid stage." },
            { status: 400 }
        );
    }
    ...
    if (isStale){
        const res1 = await fetch(
            `https://api.football-data.org/v4/competitions/DED/matches?stage=${stage}`,
            {
                headers: {
    ...
    // Return all matches for this stage
    const [matches, total] = await Promise.all([
        ...
        prisma.match.count({ where: { stage: stage } })
    ]);
